...
def safe_model_record(row: dict[str, Any], slot: int) -> dict[str, Any]:
    record = {
        ...
        "user_history": str(row["user_history"]),
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--prompt-template",
        type=Path,
        default=Path("experiments/prompts/codex_target_free_cot_fallback_v1.txt"),
    )
    parser.add_argument(
        "--schema",
        type=Path,
        default=Path("experiments/prompts/codex_target_free_cot_fallback_v1.schema.json"),
    )
